...
namespace cameras
{
  ...
  GridCalibrationTargetAprilgrid::GridCalibrationTargetAprilgrid(
      int tagRows, int tagCols, double tagSize, double tagSpacing,
  ...
  void GridCalibrationTargetAprilgrid::initialize()
  {
    ...
    _tagDetector = new AprilTags::TagDetector(_tagCodes, _options.blackTagBorder);
  }
      ...
      std::vector<bool> &outCornerObserved) const
  {
    ...
    if (_options.showExtractionVideo)
    {
      ...
      cv::imwrite("/data/suween/corner_detect/data/result/Tag_corners.jpg", imageCopy1);
      ...
      // cv::waitKey(1);
      //std::cout << "saving image ...." << std::endl;
      cv::imwrite("/data/suween/corner_detect/data/result/Tag_corners_modification.jpg", imageCopy2);
    ...
    for (unsigned int i = 0; i < detections.size(); i++)
    {
      ...
      for (int j = 0; j < 4; j++)
      {
        ...
        double subpix_displacement_squarred = (corner_x - cornerRaw_x) * (corner_x - cornerRaw_x) + (corner_y - cornerRaw_y) * (corner_y - cornerRaw_y);
